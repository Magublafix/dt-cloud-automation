...
parser.add_argument("-am", "--auto-monaco", dest="autoMonaco", help="Use this to automatically execute monaco to deploy dashboards. (missing = false)",
                    action="store_false")
parser.add_argument("-sre", "--sre-tag", dest="sre", help="Use this to only include services tagged with 'sre' as part of the serviceflow.",
                    action="store_false")
...
parser.add_argument("-l", "--logging", action="store", choices=["DEBUG", "INFO", "ERROR"], default="INFO",
                    help="Optional logging levels, default is INFO if nothing is specified")
...
logging.basicConfig(stream=sys.stderr, format="%(asctime)s [%(levelname)s] %(message)s",
                    datefmt='%Y-%m-%d %H:%M:%S')  # Sets logging format to "[LEVEL] log message"
...
def calculateDepthRelationship(layer: Dict, url: str, api: Dict, callee: Dict, startId: str, index=0, initialLevel=2):
    ...
    if relationshipCalls is None or index == len(relationshipCalls):
        ...
        if sre:
            httpResult = handleGet('{url}/api/v2/entities'.format(url=url), api,
                                   {"entitySelector": entitySelector, "from": timeFrame, "fields": "tags,fromRelationships.calls,properties.serviceType"},
                                   logger)
        else:
            httpResult = handleGet('{url}/api/v2/entities'.format(url=url), api, {"entitySelector": entitySelector + ",tag(sre)", "from": timeFrame,
                                                                                  "fields": "tags,fromRelationships.calls,properties.serviceType"}, logger)
        ...
        if index > 20:
            ...
            if initialLevel in SERVICER:
                SERVICER[initialLevel] = addServiceInOrder(SERVICER[initialLevel], id,
                                                           {'id': id, 'name': temp['displayName'], 'servicetype': temp["properties"]['serviceType'],
                                                            "requestCount": requestCount, "baseline": baseline}, requestCount)
                if (temp["properties"]["serviceType"] != 'DATABASE_SERVICE'):
                    LEVEL[initialLevel] += requestCount
            else:
                LEVEL[initialLevel] = requestCount
                SERVICER[initialLevel] = {}
                SERVICER[initialLevel][id] = {'id': id, 'name': temp['displayName'], 'servicetype': temp["properties"]['serviceType'],
                                              "requestCount": requestCount, "baseline": baseline}
...
def buildProject(name, owner, shared, preset, timeFrame, finalDash):
    dashboardYaml = {'config': [{name: "dashboard.json"}],
                     name: [{"name": "[4-Golden-Signals] {name}-serviceflow".format(name=name)}, {"owner": owner}, {"shared": shared}, {"preset": preset},
                            {"timeFrame": timeFrame}]}
...
def getBaseline(url, api, serviceType, id, timeFrame, warnP, passP):
    ...
    getMetric = handleGet('{url}/api/v2/metrics/query'.format(url=url), api,
                          {"metricSelector": metricSelector, "resolution": "INF", "entitySelector": "type(service),entityId({id})".format(id=id),
                           "from": timeFrame}, logger)
        ...
        for i in getMetric["result"]:
            if not i["data"]:
                ...
                if base == 0:
                    ...
                    if i[
                        "metricId"] in "builtin:service.response.time,builtin:service.cpu.perRequest,builtin:service.errors.total.rate,builtin:service.dbconnections.failureRate":
...
def serviceflow():
    ...
    resultJ = handleGet('{url}/api/v2/entities'.format(url=url), api, {"entitySelector": "type(service),entityId({id})".format(id=id), "from": timeFrame,
                                                                       "fields": "tags,fromRelationships.calls,properties.serviceType"}, logger)
    if resultJ["entities"]:
